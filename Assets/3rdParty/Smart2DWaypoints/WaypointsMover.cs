...
namespace Assets.Plugins.Smart2DWaypoints.Scripts
{
    public class WaypointsMover : MonoBehaviour
    {
        ...
        public System.Action<int> OnPointReached;
        ...
        private void AlignToDirection()
        {
            if (IsAlignToDirection && Direction != Vector3.zero)
            {
                ...
                TrsChildItem.localScale = new Vector3(1, Direction.x >= 0 ? 1 : -1, 1);
            }
        }
        ...
        protected IEnumerator OnWaypointReached(float timeOffset = 0)
        {
            if (OnPointReached != null)
                OnPointReached.Invoke(_targetIndex);
            ...
            _pathPartStartTime = Time.time - _nextPartTime - timeOffset * _pathPartAllTime;
            _nextPartTime = 0;
        ...
        public void GoDealwith(Path path, int index)
        {
            Path = path;
            Path.AddMover2Dic(this);
            StartRunning();
            StartWaypoint = path.Waypoints[GetBeginPointIndexByIndex(index)];
            _targetIndex = Path.GetIndex(StartWaypoint);
            Transform.position = Path.CapturePosition(this, _targetIndex);
            StartCoroutine(OnWaypointReached(GetOffsetTimeByIndex(index)));
        }

        float GetOffsetTimeByIndex(int index)
        {
            return index % 2 * 0.5f;
        }

        int GetBeginPointIndexByIndex(int index)
        {
