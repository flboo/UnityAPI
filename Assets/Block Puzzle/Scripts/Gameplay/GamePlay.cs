                        ...
                        if (!data.isFilled)
                        {
                            ...
                            foreach (BlockShapeDetails s in ObjectBlocks)
                            {
                                ...
                                if (chkBlock != null && !chkBlock.isFilled)
                                {
                                    ...
                                    GameDataManager.instance.createBlockElement(chkBlock.rowId, chkBlock.columnId, ColorToSet);
                                }
                            }
        ...
        if (BlocksToDestroy != null && BlocksToDestroy.Count > 0)
        {
            ...
            for (int i = 0; i < BlocksToDestroy.Count; i++)
            {
                foreach (BlockData d in BlocksToDestroy[i])
                {
                    ...
                    GameDataManager.instance.createBlockElement(d.rowId, d.columnId, blockColor);
        ...
        if (DoPlaceBomb && bombPlacingDetails != null)
        {
            if (DoPlaceBomb)
            {
                for (int i = 0; i < NoOfBombToPlace; i++)
                {
                    ...
                    GameObject Text = (GameObject)Instantiate(transform.Find("GamePlay-Content/txt-Bomb").gameObject);
