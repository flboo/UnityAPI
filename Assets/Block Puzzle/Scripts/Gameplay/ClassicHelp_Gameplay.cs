...
public class ClassicHelp_Gameplay : MonoBehaviour
{
	...
	void Start()
	{
		HelpContent = transform.Find ("GamePlay-Content").Find ("Content");
		blockContainer = BlockTrayManager.instance.blockContainer;
		helpMode = "Horizontal";
		PlaceObject ();
		Invoke ("ChangeBlockColor", 0.1F);
		HelpContent.Find ("txt-help1").gameObject.SetActive (true);
		handImage = transform.Find ("GamePlay-Content/img-hand").transform;
		Invoke ("HandAnimation", 1f);
	}

	void HandAnimation()
	{
		if (HelpContent.Find ("txt-help1").gameObject.activeSelf) {
	...
	void ChangeBlockColor()
	{
		...
		if (helpMode != "Horizontal") {
			transform.Find ("GamePlay-Content/img-rect").transform.eulerAngles = new Vector3 (0, 0,90);
		}
	}

	public void OnRowComplete()
	{
		HelpContent.Find ("txt-help1").gameObject.SetActive (false);
		HelpContent.Find ("txt-help3").gameObject.SetActive (true);
		HelpContent.Find ("txt-help2").gameObject.SetActive (true);
		helpMode = "Verticle";
	...
	void ResetBlockContainer()
	{
		...
		HelpContent.Find ("txt-help3").gameObject.SetActive (false);
		HelpContent.Find ("txt-help2").gameObject.SetActive (false);
		HelpContent.Find ("txt-help4").gameObject.SetActive (true);
		Invoke ("HandAnimation", 1f);
	}

	public void OnColumnComplete()
	{
		HelpContent.Find ("txt-help4").gameObject.SetActive (false);
		HelpContent.Find ("txt-help5").gameObject.SetActive (true);
		transform.Find ("GamePlay-Content/img-rect").gameObject.SetActive (false);
